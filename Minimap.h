...
struct Minimap {
    ...
    void render(unsigned int shader2D, const Map& map, const Camera& camera, bool fullScreenMode, float currentAlpha, int targetW, int targetH) {
        ...
        float xTop = pixelToNdcX(pXTop, denomW); float yTop = pixelToNdcY(pYTop, denomH);
        float xLeft = pixelToNdcX(pXLeft, denomW); float yLeft = pixelToNdcY(pYLeft, denomH);
        float xRight = pixelToNdcX(pXRight, denomW); float yRight = pixelToNdcY(pYRight, denomH);
