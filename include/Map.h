...
struct Map {
    ...
    int isWall(int x, int z) {
        if (x < 0 || x >= MAP_SIZE || z < 0 || z >= MAP_SIZE) return 1;
        return grid[x][z];
    }
    ...
    void createCorridor(int x1, int z1, int x2, int z2) {
        int startX = std::min(x1, x2); int endX = std::max(x1, x2);
        for (int x = startX; x <= endX; ++x) grid[x][z1] = 0;
        int startZ = std::min(z1, z2); int endZ = std::max(z1, z2);
        for (int z = startZ; z <= endZ; ++z) grid[x2][z] = 0;
    }

    void generate() {
        for (int x = 0; x < MAP_SIZE; x++) {
            for (int z = 0; z < MAP_SIZE; z++) {
                grid[x][z] = 1; visible[x][z] = 0;
            }
        }
        ...
        for (int rX = 0; rX < 3; ++rX) {
            for (int rZ = 0; rZ < 3; ++rZ) {
                int minX = rX * zoneSize + 2; int minZ = rZ * zoneSize + 2;
                int maxW = zoneSize - 4;      int maxH = zoneSize - 4;
                int w = std::rand() % 4 + 4;  int h = std::rand() % 4 + 4;
                ...
                for (int tx = room.x; tx < room.x + room.w; ++tx) {
                    for (int tz = room.z; tz < room.z + room.h; ++tz) grid[tx][tz] = 0;
                }
        ...
        for (int x = 0; x < MAP_SIZE; x++) {
            for (int z = 0; z < MAP_SIZE; z++) {
                if (grid[x][z] == 1) addVisibleFaces(x, z);
            }
        }
